...
;*loop over 2-minute intervals within 20-minute interval and calculate lwp std
 ...
 lwp_off_x = -999.
 FOR j = 0, 9 DO BEGIN
  i_two = WHERE(time GE twe_min(i)+FLOAT(j)*two_min AND time LT twe_min(i)+FLOAT(j+1)*two_min AND ele GT ele_min AND ele LT ele_max AND lwp NE -999. AND flag_2a EQ 0., nn)
  IF nn GE 30. THEN BEGIN
;print, twe_min(i)+FLOAT(j)*two_min, twe_min(i)+FLOAT(j+1)*two_min 
...
;print,date,'number of clearsky lwps: ',n_off
;print, time_off
;print, lwp_off
...
FOR i = 0l, n_lwp-1l DO BEGIN
 ii_high = WHERE(time_off GE jd_lwp(i) AND lwp_off NE -999., n_high)
 ii_low = WHERE(time_off LT jd_lwp(i) AND lwp_off NE -999., n_low)
 ii_ok = WHERE(time_off NE -999.)
 CLOSEST, jd_lwp(i), time_off(ii_ok), dmin, ind
 ii_closest = ii_ok(ind)
...
;*off-zenith lwp are NOT corrected
  IF ele(i) GT ele_min AND ele(i) LT ele_max THEN lwp_cor(i) = lwp(i) - offset
