...
; $Id: $
; Abstract:
; * calculation of TB offset correction using COSMO-DE, ICON or AROME output
...
;**search for model output in case of clear-sky cases
   IF clear EQ 1 AND cloudy EQ 0 THEN BEGIN
    ...
    IF opened EQ 0 AND model_choice EQ 'cosmo' THEN BEGIN
     ...
     opened = 1
    ENDIF ELSEIF opened EQ 0 AND model_choice EQ 'arome' THEN BEGIN

HIER WEITER
    ENDIF ELSEIF opened EQ 0 AND model_choice EQ 'icon' THEN BEGIN
