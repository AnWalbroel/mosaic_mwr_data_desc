...
; 2015-09-28 (UL):
; * multiple calls of this routine resulted in multiple multiplications of tb_p 
; * --> relevant when including surface measurements; corrected by introducing tp_px
; 2021-07-21 (AW):
; * added cubic regression coefficient to GET_COEFF_LEVEL2A_NC (c_c)
;-
...
ENDIF ELSE IF ret_file EQ 'nc' THEN BEGIN
 ...
 GET_COEFF_LEVEL2A_NC, 0, ret_files(0), ret_par, angle, f, offset, c_l, c_q, c_c, c_s, predictand_err
...
IF ret_par.surf EQ 'in_surface' THEN BEGIN 
 ii = WHERE(tb_t GT 250. AND tb_t LT 330. AND tb_q GE 0. AND tb_q LT 0.02 AND tb_px GT 0. AND tb_px LT 120000.)  
 T_help = tb_t(ii)
 q_help = tb_q(ii)
 p_help = tb_px(ii)
 time_help = time(ii)
 ...
 FOR i = 0l, N_ELEMENTS(time)-1l DO BEGIN
  IF tb_t(i) GT 250. AND tb_t(i) LT 330. AND tb_q(i) GE 0. AND tb_q(i) LT 0.02 AND $
     tb_px(i) GT 0. AND tb_px(i) LT 120000. THEN BEGIN
;  OK!!
  ENDIF ELSE BEGIN
   CLOSEST, time(i), time_help, dmin, index
   tb_t(i) = T_help(index)
   tb_q(i) = q_help(index)
   tb_px(i) = p_help(index)
  ENDELSE 
 ENDFOR

 surf = TRANSPOSE([[tb_t], [tb_q], [tb_px]])
ENDIF
...
FOR j = 0, n_ret-1 DO BEGIN
 ...
 ENDIF ELSE IF ret_file EQ 'nc' THEN BEGIN
  ...
  GET_COEFF_LEVEL2A_NC, 0, ret_files(j), ret_par, angle, f, offset, c_l, c_q, c_c, c_s, predictand_err
...
;**linearly interpolate regression coefficients to elevation angle of measurement
  FOR j = 0, n_ret-2 DO BEGIN
   ...
   IF elev(i) GE angle_c(j)-0.6 AND elev(i) LE angle_c(j+1)+0.6 THEN BEGIN
    ...
    FOR k = 0, n_f-1 DO BEGIN
     ...
     steig_c = (c_c_a(k, j+1)-c_c_a(k, j))/(angle_a(j+1)-angle_a(j))
     c_c_x(k) = c_c_a(k, j) + steig_c*(elev(i)-angle_a(j))
    ENDFOR
    IF ret_par.surf EQ 'in_surface' THEN BEGIN
     FOR k = 0, 2 DO BEGIN
    ...
    IF ret_par.reg EQ 'cubic' THEN BEGIN
     predictand(i) = offset_x + TOTAL(c_l_x*tb_algo(0:n_f-1, i)) + TOTAL(c_q_x*(tb_algo(0:n_f-1, i)^2)) + TOTAL(c_c_x*(tb_algo(0:n_f-1, i)^3))
     dpredictand(i) = -1.*(predictand(i) - (offset_x + TOTAL(c_l_x*tb_algo_org(0:n_f-1, i)) + TOTAL(c_q_x*(tb_algo_org(0:n_f-1, i)^2)) + TOTAL(c_c_x*(tb_algo_org(0:n_f-1, i)^3))))
     IF ret_par.surf EQ 'in_surface' THEN BEGIN
      predictand(i) = offset_x + TOTAL(c_l_x*tb_algo(0:n_f-1, i)) + TOTAL(c_q_x*(tb_algo(0:n_f-1, i)^2)) + TOTAL(c_c_x*(tb_algo(0:n_f-1, i)^3)) + TOTAL(c_s_x*surf(*, i))
      dpredictand(i) = -1.*(predictand(i) - (offset_x + TOTAL(c_l_x*tb_algo_org(0:n_f-1, i)) + TOTAL(c_q_x*(tb_algo_org(0:n_f-1, i)^2)) + TOTAL(c_c_x*(tb_algo_org(0:n_f-1, i)^3)) + TOTAL(c_s_x*surf(*, i))))
     ENDIF
    ENDIF
 ...
 ENDIF ELSE BEGIN
  ...
  IF n_ret EQ 1 AND elev(i) GT angle_a(0)-0.6 AND elev(i) LT angle_a(0)+0.6 THEN BEGIN
   ...
   IF ret_par.reg EQ 'cubic' THEN BEGIN
    predictand(i) = offset_a(0) + TOTAL(c_l_a(*, 0)*tb_algo(0:n_f-1, i)) + TOTAL(c_q_a(*, 0)*(tb_algo(0:n_f-1, i)^2)) + TOTAL(c_c_a(*, 0)*(tb_algo(0:n_f-1, i)^3))
    dpredictand(i) = -1.*(predictand(i) - (offset_a(0) + TOTAL(c_l_a(*, 0)*tb_algo_org(0:n_f-1, i)) + TOTAL(c_q_a(*, 0)*(tb_algo_org(0:n_f-1, i)^2)) + TOTAL(c_c_a(*, 0)*(tb_algo_org(0:n_f-1, i)^3))))
    IF ret_par.surf EQ 'in_surface' THEN BEGIN
     predictand(i) = offset_a(0) + TOTAL(c_l_a(*, 0)*tb_algo(0:n_f-1, i)) + TOTAL(c_q_a(*, 0)*(tb_algo(0:n_f-1, i)^2)) + TOTAL(c_c_a(*, 0)*(tb_algo(0:n_f-1, i)^3)) + TOTAL(c_s_a(*, 0)*surf(*, i))
     dpredictand(i) = -1.*(predictand(i) - (offset_a(0) + TOTAL(c_l_a(*,0)*tb_algo_org(0:n_f-1,i)) + TOTAL(c_q_a(*,0)*(tb_algo_org(0:n_f-1,i)^2)) + TOTAL(c_c_a(*,0)*(tb_algo_org(0:n_f-1,i)^3)) + TOTAL(c_s_a(*, 0)*surf(*, i))))
    ENDIF
   ENDIF
