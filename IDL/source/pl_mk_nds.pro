...
mwr_name=mwr_name,$                 ; MWR instrument name
meas_site=meas_site,$               ; site
...
; 2015-07-09 (UL)
; minor corrections concerning plotting ranges of level2a plots
; 2016-07-05 (UL)
; NDS: adpated paths to match new data structure agreed upon on at retreat 2015
; 2017-05-31 (UL)
; Solar inference flags updated - azimith dependence for high elevation angles disregarded
; 2018-08-23 (UL)
; SPC check option included
;-
...
date_mm = STRMID(date, 2, 2)
date_dd_c = STRMID(date, 4, 2)
date_dd = FIX(STRMID(date, 4, 2)) 
...
;****all files for one day (raw, formatted, plots) located in one directory for NDS
raw_path = mwr_dir+'/l1/'+yyyy+'/'+date_mm+'/'+date_dd_c+'/'
raw_path_prev = mwr_dir+'/l1/'+yyyy_prev+'/'+mm_prev+'/'+dd_prev+'/'

filetest = FILE_SEARCH(raw_path+'/*.*')

IF filetest(0) EQ '' THEN BEGIN
 print, 'NO raw data found for '+date
ENDIF

plot_path1 = mwr_dir+'/l1/'+yyyy+'/'+date_mm+'/'+date_dd_c+'/'
plot_path2 = mwr_dir+'/l2/'+yyyy+'/'+date_mm+'/'+date_dd_c+'/'

data_path0 = mwr_dir+'/l0/'+yyyy+'/'+date_mm+'/'+date_dd_c+'/'
data_path1 = mwr_dir+'/l1/'+yyyy+'/'+date_mm+'/'+date_dd_c+'/'
data_path2 = mwr_dir+'/l2/'+yyyy+'/'+date_mm+'/'+date_dd_c+'/'
...
tb_spc = {time:-999., tb:[[-999.], [-999.]], el:-999., az:-999., f:-999., r:-999.}
...
IF rd_file EQ 'wvl' OR rd_file EQ 'brt' THEN BEGIN
 GET_RPG, raw_path, raw_path_prev, ang_low, ang_high, rd_file, date, tb, plot, l1c, met, hkd, irt, tb_spc, verbose=verbose
...
;***azimuth correction for HATPRO level1b (transform to "geographical" coordinates)
 ...
 IF az_cor NE -999. THEN BEGIN
  ...
  i_gt360 = WHERE(az GT 360.)
  IF i_gt360(0) NE -1 THEN az(i_gt360) = az(i_gt360) - 360.
 ENDIF
...
;***azimuth correction for HATPRO level1c (transform to "geographical" coordinates)
 ...
 IF az_cor NE -999. THEN BEGIN
  ...
  i_gt360 = WHERE(az_l1c GT 360.)
  IF i_gt360(0) NE -1 THEN az_l1c(i_gt360) = az_l1c(i_gt360) - 360.
 ENDIF
...
;- FLAG values
;Bit1: VISUAL INSPECTION Band1 (user edited filter_*.dat file)
;Bit2: VISUAL INSPECTION Band2 (user edited filter_*.dat file)
;Bit3: VISUAL INSPECTION Band3 (user edited filter_*.dat file)
;Bit4: RAIN FLAG (RPG specific)
;Bit5: SANITY RECEIVER Band1 (RPG specific)
;Bit6: SANITY RECEIVER Band2 (RPG specific)
;Bit7: solar flag
;Bit8: TB SANITY THRESHOLD Band1 (set in par_mwr_pro.pro)
;Bit9: TB SANITY THRESHOLD Band2 (set in par_mwr_pro.pro)
;Bit10: TB SANITY THRESHOLD Band3 (set in par_mwr_pro.pro)
...
filter_file = FILE_SEARCH(mwr_dir+'/filter_'+meas_site+'_'+mwr_name+'.dat')
...
IF i_check_sol(0) NE -1 THEN BEGIN
 FOR i = 0, n_check_sol-1 DO BEGIN
  j = i_check_sol(i)
  IF tb.el(j) LT 90.-saf OR tb.el(j) GT 90.+saf THEN BEGIN
   ...
   ENDIF
  ENDIF ELSE BEGIN ; for elevation angles close to zentih disregard matching azimuth angles
   IF (tb.el(j) GE sol_el(j)-saf AND tb.el(j) LE sol_el(j)+saf) THEN BEGIN
    flag_1b(j) = flag_1b(j) OR 64
    flag_2a(j) = flag_2a(j) OR 64
    flag_T2b(j) = flag_T2b(j) OR 64
    flag_q2b(j) = flag_q2b(j) OR 64
...
IF i_check_sol(0) NE -1 THEN BEGIN
 FOR i = 0, n_check_sol-1 DO BEGIN
  j = i_check_sol(i)
  IF l1c.el(j) LT 90.-saf OR l1c.el(j) GT 90.+saf THEN BEGIN
   ...
   ENDIF
  ENDIF ELSE BEGIN ; for elevation angles close to zentih disregard matching azimuth angles
   IF (l1c.el(j) GE sol_el(j)-saf AND l1c.el(j) LE sol_el(j)+saf) THEN BEGIN
    flag_1c(j) = flag_1b(j) OR 64
    flag_2c(j) = flag_2a(j) OR 64
   ENDIF
  ENDELSE
...
IF ISA(use_spc) EQ 0 THEN use_spc = 0
...
;**SPC check
 IF use_spc EQ 1 THEN BEGIN
  spc_thres_file1 = FILE_SEARCH(mwr_dir+'/'+spc_thres_file)
  IF N_ELEMENTS(spc_thres_file1) EQ 0  THEN BEGIN
   print, 'No spc_thres_file file found'
  ENDIF ELSE BEGIN
   RESTORE, file = spc_thres_file1
   n_ele_spc = N_ELEMENTS(ele_spc)

;*check for available frequency channels in spc_thres file

   FOR i = 0, nfreq_check-1 DO BEGIN 
 
    ii = WHERE(tb_spc.f GT tb.f(ifreq_check(i))-1e-5 AND tb_spc.f LT tb.f(ifreq_check(i))+1e-5, n_ii)
    ii = ii(0)
    IF n_ii GT 1 THEN BEGIN
     CLOSEST, tb.f(ifreq_check(i)), tb_spc.f, dmin, ii
     n_ii = 1
    ENDIF

    IF n_ii EQ 1 THEN BEGIN

     FOR j = 0, n_ele_spc-1 DO BEGIN
     
      jj = WHERE(tb.el GT ele_spc(j)-0.6 AND tb.el LT ele_spc(j)+0.6, n_jj) 
      jjj = WHERE(tb_spc.el GT ele_spc(j)-0.6 AND tb_spc.el LT ele_spc(j)+0.6, n_jjj) 

      IF jj(0) NE -1 AND jjj(0) NE -1 THEN BEGIN 
       kk = WHERE(ABS(tb.tb(ifreq_check(i), jj)-tb_spc.tb(ii, jj)) GT thres_spc(ii, j)*spc_fac, n_kk)

       IF kk(0) NE -1 THEN BEGIN
        flag_1b(jj(kk)) = flag_1b(jj(kk)) OR 128
        flag_2a(jj(kk)) = flag_2a(jj(kk)) OR 128
        flag_T2b(jj(kk)) = flag_T2b(jj(kk)) OR 128   
        flag_q2b(jj(kk)) = flag_q2b(jj(kk)) OR 128  
        IF verbose THEN BEGIN
         print, 'Spectral inconsistency at frequency: ', tb.f(ifreq_check(i))
         print, 'Elevation angle: ', ele_spc(j)
         print, 'Measurements (#): ', n_kk
        ENDIF
       ENDIF  
      ENDIF
     ENDFOR
    ...
    IF n_ii EQ 1 THEN BEGIN
     ...
     FOR j = 0, n_ele_spc-1 DO BEGIN
      ...
      IF jj(0) NE -1 AND jjj(0) NE -1 THEN BEGIN
       ...
       IF kk(0) NE -1 THEN BEGIN
        flag_1b(jj(kk)) = flag_1b(jj(kk)) OR 256
        flag_2a(jj(kk)) = flag_2a(jj(kk)) OR 256
        flag_T2b(jj(kk)) = flag_T2b(jj(kk)) OR 256   
        flag_q2b(jj(kk)) = flag_q2b(jj(kk)) OR 256
        IF verbose THEN BEGIN
         print, 'Spectral inconsistency at frequency: ', tb.f(ifreq_check(i))
         print, 'Elevation angle: ', ele_spc(j)
         print, 'Measurements (#): ', n_kk
   ...
   FOR i = 0, nfreq_check-1 DO BEGIN 
    ...
    IF n_ii EQ 1 THEN BEGIN
     ...
     FOR j = 0, n_ele_spc-1 DO BEGIN
      ...
      IF jj(0) NE -1 AND jjj(k) NE -1 THEN BEGIN 

       kk = WHERE(ABS(tb.tb(ifreq_check(i), jj)-tb_spc.tb(ii, jj)) GT thres_spc(ii, j)*spc_fac, n_kk)

       IF kk(0) NE -1 THEN BEGIN
        flag_1b(jj(kk)) = flag_1b(jj(kk)) OR 512
        flag_2a(jj(kk)) = flag_2a(jj(kk)) OR 512
        flag_T2b(jj(kk)) = flag_T2b(jj(kk)) OR 512   
        flag_q2b(jj(kk)) = flag_q2b(jj(kk)) OR 512  
        IF verbose THEN BEGIN
         print, 'Spectral inconsistency at frequency: ', tb.f(ifreq_check(i))
         print, 'Elevation angle: ', ele_spc(j)
         print, 'Measurements (#): ', n_kk
...
;**classical TB threshold check
 ...
 FOR i = 0, nfreq_check-1 DO BEGIN
  ...
  IF jj(0) NE -1 THEN BEGIN
   ...
   flag_T2b(jj) = flag_T2b(jj) OR 512   
...
ENDIF ELSE lwp_cor=lwp ; set_lwp_off eq 1 (if set_lwp_off EQ 0, use original lwp in clwvi)
...
;***transform relative humdity go from % to [1]
 ...
 IF ISA(zoo) EQ 0 THEN zoo = 0

 IF zoo EQ 1 THEN BEGIN
  WRITE_LEVEL1B_ZO, data_path1, date, mwr_meas, latitude, longitude, altitude, tb.f, tb.time, time_bnds_oz, tb_l1b_oc, l1b_oc, offset_tb_type, freq_shift,$
                    flag_1b, tb_t, tb_p, tb_rh, az, el, thresholds(0:1), irt.wavel, tb_irt, el_irt, file_naming_l1b, global_atts, tb_bias, tb_cov 
 ENDIF
...
;*************
 IF N_ELEMENTS(algo_iwv) NE 0 OR N_ELEMENTS(algo_lwp) NE 0 OR N_ELEMENTS(algo_wdl) NE 0 OR N_ELEMENTS(algo_taa) NE 0 THEN BEGIN  
  ...
  WRITE_LEVEL2A, data_path2, date, mwr_meas, latitude, longitude, altitude, tb.time, time_bnds_o, flag_2a,$
                 ...
                 lwp_std_thres=lwp_std_thres, set_lwp_off=set_lwp_off
 ENDIF
...
DEVICE, /color, /landscape, filename = plot_path1+date+'_'+meas_site+'_l1.ps'
...
;*overplot flagged times
 ...
 IF N_ELEMENTS(met.time) GT 1 THEN BEGIN
  XYOUTS, 0.532, 0.985, 'Measurement: '+ meas_site + '_' + mwr_name + ', '+ date + ang_range, alignment=0.5, charsize=1.2, /NORMAL
 ENDIF ELSE BEGIN
  XYOUTS, 0.532, 0.785, 'Measurement: '+ meas_site + '_' + mwr_name + ', '+ date + ang_range, alignment=0.5, charsize=1.2, /NORMAL
 ENDELSE
...
;**plot environmental T & RH values if available
 IF N_ELEMENTS(met.time) GT 1 THEN BEGIN
  ...
  OPLOT, met.time, met.q, psym = 3
   ...
   indextemp=WHERE(met.t GT 230 and met.t LT 330, counttemp)
   IF counttemp GT 1 THEN BEGIN
   dy = MAX(met.t(indextemp)) - MIN(met.t(indextemp))
   ymin = MIN(met.t(indextemp)) - dy/10.
   ymax = MAX(met.t(indextemp)) + dy/10.
   ENDIF ELSE BEGIN
   ymin=260.
   ymax=300.
   ENDELSE
...
;***plot tb spectra
if verbose then print,"creating '",date+'_'+meas_site+'_l1_sp.ps','"'
!p.font =0
!P.Charsize=1.2
!P.thick = 1.3
!X.minor = 5
...
SET_PLOT, 'PS'
;DEVICE, /color, /landscape, filename = plot_path1+date+'_'+meas_site+'_l1_sp.ps'
DEVICE, /color, /portrait, filename = plot_path1+date+'_'+meas_site+'_l1_sp.ps'
...
tit = 'Measurement: '+meas_site + '_' + mwr_name + ', ' + date +': TB daily means +/-STDEV' + ang_range
...
;***plot angles & flags
IF verbose then print,"creating '",date+'_'+meas_site+'_flag.ps','"'
!p.font = 0
!P.Charsize = 1.8
!P.thick = 1.5
!P.multi = 14
...
SET_PLOT, 'PS'
DEVICE, /color, /landscape, filename = plot_path1+date+'_'+meas_site+'_flag.ps'
...
;**plot azimuth
PLOT, tb.time, az, yrange=[0., 360.], ystyle=1, title = 'Angles and flags: ' + meas_site+$ 
'_' + mwr_name + ', ' + date, ytickv=1, ytitle='Azimuth [deg]', ycharsize = 0.8, xticklen = 0.07,$
xrange=[0.,24.], xstyle=1, xcharsize = 1e-5, xtickinterval = 6,$
...
index = WHERE((flag_T2b AND 2^i) EQ 2^i, count)
IF count GT 0 THEN BEGIN
 IF count EQ 1 THEN BEGIN
  count = 2
  index = [index(0), index(0)]
...
data_2a = REPLICATE(-999., 3, N_ELEMENTS(iwv))
...
i_ang = WHERE(tb.el GT ang_low AND tb.el LT ang_high AND iwv GE 0. AND lwp_cor GT -0.2, n_i_ang)
...
IF n_i_ang GT 1 THEN BEGIN
 ...
 IF N_ELEMENTS(algo_iwv) NE 0 THEN BEGIN
  algos_2a(0) = 'iwv'
  data_2a(0, i_ang) = iwv(i_ang)
 ENDIF

 IF N_ELEMENTS(algo_lwp) NE 0 THEN BEGIN
  algos_2a(1) = 'lwp'
  data_2a(1, i_ang) = lwp(i_ang)*1000.
  IF set_lwp_off EQ 1 THEN data_2a(1, i_ang) = lwp_cor(i_ang)*1000.
 ENDIF
 IF N_ELEMENTS(algo_wdl) NE 0 THEN BEGIN
  algos_2a(2) = 'wdl'
  data_2a(2, i_ang) = wdl(i_ang)
 ENDIF
 ...
 IF n_algos_2a(0) NE -1 THEN BEGIN
  ...
  IF verbose then print,"creating '",date+'_'+meas_site+'_l2a.ps','"'
  !p.font = 0
  !P.Charsize = 1.8
  !P.thick = 1.3
  !X.minor = 6
  ...
  SET_PLOT, 'PS'
  DEVICE, /color, /landscape, filename = plot_path2+date+'_'+meas_site+'_l2a.ps'
...
;**plot environmental T & RH values if available
  IF N_ELEMENTS(met.time) GT 1 THEN BEGIN
   tit = 'Measurement: '+meas_site + '_' + mwr_name + ', '+ date + ang_range
   PLOT, met.time, met.q, yrange=[0. ,100.], ystyle=1, title = tit,$
  ...
  FOR i = 0, n_algos_2a-1 DO BEGIN
   ...
   IF plot_range EQ 'flag' AND count GT 1 THEN BEGIN
    dy = MAX(data_2a(i, i_ang(index))) - MIN(data_2a(i, i_ang(index)))
    ymin = MIN(data_2a(i, i_ang(index))) - dy/10.
    ymax = MAX(data_2a(i, i_ang(index))) + dy/10.
   ENDIF ELSE BEGIN
    dy = MAX(data_2a(i, i_ang)) - MIN(data_2a(i, i_ang))
    ymin = MIN(data_2a(i, i_ang)) - dy/10.
    ymax = MAX(data_2a(i, i_ang)) + dy/10.
   ENDELSE
   IF algos_2a(i) EQ 'iwv' THEN BEGIN
   IF ymax GT 30. THEN ymax=30.
   ENDIF
...
;*make plot frames
   ...
   IF y_min(ii) EQ MIN(y_min) OR i EQ n_algos_2a-1 THEN BEGIN
    ...
    PLOT, tb.time, data_2a(i, *), yrange=[ymin, ymax], ystyle = 1,xtickinterval=6,$
   ...
   ENDIF ELSE BEGIN
    tit = ''
    IF i EQ 0 THEN BEGIN
     IF N_ELEMENTS(met.time) EQ 0 THEN tit = 'Measurement: '+meas_site + '_' + mwr_name + ', '+ date + ang_range
     xtl = 0.02
    ENDIF
    PLOT, tb.time, data_2a(i, *), yrange=[ymin, ymax], ystyle = 1,xtickinterval=6,$
    xrange = [0., 24.], xstyle=1, xtitle = 'Time (UTC) [h]', ytitle = ytit_2a(i),$
...
;*plot level2a data
   IF plot_range EQ 'flag' AND count GT 1 THEN BEGIN
    OPLOT, tb.time(i_ang(index)), data_2a(i, i_ang(index)), psym = 3
   ENDIF ELSE IF plot_range EQ 'all' THEN BEGIN
    OPLOT, tb.time, data_2a(i, *), psym = 3
   ENDIF
...
;*calc and plot mean & stddev
   ...
   IF count GT 1 THEN BEGIN
    m = MEAN(data_2a(i, i_ang(index)))
    str = STRING(m, FORMAT='(F6.2)') + ' +/- '+$
          STRING(STDDEV(data_2a(i, i_ang(index))),FORMAT='(F6.2)') + units_2a(i)
   ENDIF
   XYOUTS, x_min(ii)+0.2, y_max(ii)-0.02, str, /NORMAL, size=0.7, color=2
...
IF N_ELEMENTS(algo_iwv) GT 0 AND aztp_iwv EQ 1 THEN BEGIN
 filename = plot_path2+date+'_'+meas_site+'_l2a_iwv_aztp.ps'
 ytit = 'IWV / kgm!e-2!N'

 AZTP, par='IWV', filename=filename, date=date, meas_site=meas_site, mwr_name=mwr_name, time=tb.time, data=iwv, ele=tb.el, azi=az, ytit=ytit, el_aztp=el_aztp,$
...
IF N_ELEMENTS(algo_lwp) GT 0 AND aztp_lwp EQ 1 THEN BEGIN
 filename = plot_path2+date+'_'+meas_site+'_l2a_lwp_aztp.ps'
 ytit = 'LWP / gm!e-2!N' 
 lwpx = lwp
 IF set_lwp_off EQ 1 THEN BEGIN
  ...
  lwpx = lwp
 ENDIF
 AZTP, par='LWP', filename=filename, date=date, meas_site=meas_site, mwr_name=mwr_name, time=tb.time, data=lwpx*1000., ele=tb.el, azi=az, ytit=ytit, el_aztp=el_aztp,$
...
IF n_i_ang GT 1 THEN BEGIN
 ...
 IF n_algos_2b(0) NE -1 THEN BEGIN
  ...
  FOR ii = 0, n_algos_2b-1 DO BEGIN
   ...
   IF verbose then print, 'creating '+date+'_'+meas_site+'_l2b_'+algo_2b(ii)+'.ps'
   SET_PLOT, 'ps'
   LOADCT,39
   !X.minor = 6

   DEVICE, /color, /landscape, filename = plot_path2+date+'_'+meas_site+'_l2b_'+algo_2b(ii)+'.ps'
   ...
   tit = ytit_2b(ii) + ' ('+ algo_2b(ii) +'), ' + meas_site + '_' + mwr_name
...
FOR i = 0, 3 DO BEGIN 
 ...
 IF verbose then print, 'creating '+date+'_'+meas_site+'_l2c_'+plots_2c(i)+'.ps'
 SET_PLOT, 'ps'
 LOADCT,39
 !X.minor = 6

 DEVICE, /color, /landscape, filename = plot_path2+date+'_'+meas_site+'_l2c_'+plots_2c(i)+'.ps'
 ...
 IF NOT FINITE(min_p) OR NOT FINITE(max_p) THEN BEGIN
  dy = MAX(data_2c(i, index_h, index)) - MIN(data_2c(i, *, index))
  min_p = MIN(data_2c(i, index_h, index)) - dy/10.
  max_p = MAX(data_2c(i, index_h, index)) + dy/10.
 ENDIF
 ...
 IF min_p LT 0. THEN min_p = 0.
 IF max_p GT 350. THEN max_p = 350.
 max_p = CEIL(max_p)
 ...
 IF i LT 2 THEN BEGIN 
  ...
  IF i EQ 0 THEN tit = 'Temperature (level2c), ' + meas_site + '_' + mwr_name 
  IF i EQ 1 THEN tit = 'Eq. pot. temperature (level2c), '+meas_site
 ...
 ENDIF ELSE IF i EQ 2 THEN BEGIN
  ...
  tit = 'Abs. humidity and IWV (level2c), '+meas_site+'_'+mwr_name

  index = WHERE(flag_2a(i_ang) EQ 0, count)
  IF plot_range EQ 'flag' AND count GT 1 THEN BEGIN
   dy = MAX(data_2a(0, i_ang(index))) - MIN(data_2a(0, i_ang(index)))
   ymin = MIN(data_2a(0, i_ang(index))) - dy/10.
   ymax = MAX(data_2a(0, i_ang(index))) + dy/10.
  ENDIF ELSE BEGIN
   dy = MAX(data_2a(0, i_ang)) - MIN(data_2a(0, i_ang))
   ymin = MIN(data_2a(0, i_ang)) - dy/10.
   ymax = MAX(data_2a(0, i_ang)) + dy/10.  
  ENDELSE
 ...
 ENDIF ELSE IF i EQ 3 THEN BEGIN
  ...
  pos_bar = [0.93, 0.5, 0.97, 0.85]
  pos_ts = [0.1, 0.7, 0.88, 0.85]
  tit = 'Rel. humidity and LWP (level2c), ' + meas_site + '_' + mwr_name
...
;**plot all radar measurements (also off-zenith measurements, azimuth and elevation angle)
 ...
 filename1_plot_radar = plot_path2 + date + '_' + meas_site +'_'  + mwr_name + '_l2_radar_lwp.ps'
 filename2_plot_radar = plot_path2 + date + '_' + meas_site + '_'  + mwr_name + '_l2_radar.ps'

 print, 'PL_MK: creating radar quicklooks: '
 print, filename1_plot_radar
 print, filename2_plot_radar

 lwp_min_rad = MIN(data_2a(1,i_ang))
 lwp_max_rad = MAX(data_2a(1,i_ang))
 ...
 PLOT_RADAR_ALL, data_path_radar, filter_file_radar, filename1_plot_radar,$
                 filename2_plot_radar, date, meas_site, plot.time, REFORM(data_2a(1,*)), lwp_min_rad, lwp_max_rad
...
IF N_ELEMENTS(ceilo_type) GT 0 AND N_ELEMENTS(algo_iwv) GT 0 AND N_ELEMENTS(algo_lwp) GT 0 THEN BEGIN
 ...
 SET_PLOT, 'ps'
 DEVICE, /color, BITS_PER_PIXEL = 8, file=plot_path2+date+'_'+meas_site+'_l2_cili.ps'
 tit = date + ', ' + meas_site + '_' + mwr_name
 !P.multi = [0, 1, 4]
 !P.charsize = 1.0
...
;*LWP plot
 ...
 IF ii(0) NE -1 THEN BEGIN
  ...
  IF plot_range EQ 'flag' AND count GT 1 THEN BEGIN
   ...
   ymax = MAX(data_2a(1, index)) + dy/10.
  ENDIF ELSE BEGIN
   dy = MAX(data_2a(1, i_ang)) - MIN(data_2a(1, i_ang))
   ymin = MIN(data_2a(1, i_ang)) - dy/10.
   ymax = MAX(data_2a(1, i_ang)) + dy/10.
  ENDELSE
