...
meas_site=meas_site,$             ; site code
mwr_name=mwr_name,$             ; instrument name code
time=time,$                     ; time 
...
;*perform air-mass correction to zenith
 ...
 IF par EQ 'IWV' AND r_max GT 70. THEN r_max = 70.
...
IF n_scan GT 1 THEN BEGIN
 ...
 FOR i_el = 0, n_el-1 DO BEGIN
  ...
  tit = par+' (az_vs_time)@'+el_c+' deg, ' + meas_site + '_' + mwr_name +', ' + date
