...
lwp_std_thres=lwp_std_thres,$   ;2-min lwp std threshold to identify clear sky - used for lwp offset correction 
set_lwp_off=set_lwp_off
; $Id: $
; Abstract:
...
; 2015-09-27
; UL: when wrinting lwp files there were still two errors
; 1.)  rfdid = NCDF_DIMDEF(outfid_iwv, 'n_ret', n_ret)
; 2.)  rfdid = NCDF_DIMDEF(outfid_iwv, 'n_ret', n_ret)
; corrected to 
; 1.)  rfdid = NCDF_DIMDEF(outfid_lwp, 'n_ret', n_ret)
; 2.)  rfdid = NCDF_DIMDEF(outfid_lwp, 'n_ret', n_ret)
; I have no idea why the complete reprocessing (2010-2015) went through without error??
; Output files seem to be OK!
; 2017-04-11 (UL)
...
IF rets(0) NE '-' THEN BEGIN
 ...
 dep_l1 = fn_l1b.kkk + '_' + fn_l1b.sss + '_' + fn_l1b.inst + fn_l1b.instnn + '_'+ fn_l1b.lll1 + '_' + fn_l1b.var + '_' + fn_l1b.vnn
...
;*****define variables & attributes
 ...
 NCDF_ATTPUT, outfid_iwv, flaid, 'flag_meanings', "visual_inspection_filter_band_1 visual_inspection_filter_band2 visual_inspection_filter_band3 rain_flag sanity_receiver_band1 sanity_receiver_band2 sun_in_beam tb_threshold_band1 tb_threshold_band2 tb_threshold_band3 iwv_lwp_threshold"
 ...
 c6 = 'clwvi (not considering clwvi offset correction) valid range: ['+ STRING(threshold(4), format = '(f4.1)') + ', ' + STRING(threshold(5), format = '(f4.1)')+'] in kgm-2; '
...
;****global attributes
 ...
 NCDF_ATTPUT, outfid_iwv, /GLOBAL, 'Comments', 'prw errors are given as a function of retrieval elevation angle. Note, actual observations may occur at different angles. Uncertainties are then interpolated.'
 NCDF_ATTPUT, outfid_iwv, /GLOBAL, 'Licence', ga.l
...
;*****define dimensions
 ...
 rfdid = NCDF_DIMDEF(outfid_lwp, 'n_ret', n_ret)
...
;*****define variables & attributes
 ...
 NCDF_ATTPUT, outfid_lwp, lwpid, 'standard_name','atmosphere_mass_content_of_cloud_liquid_water_content'
 ...
 lwpozid = NCDF_VARDEF(outfid_lwp, 'clwvi_off_zenith',[timdid], /float)
 ...
 NCDF_ATTPUT, outfid_lwp, lwpozid, 'comment', 'These values denote the path integrated amount of condensed water at arbitrary viewing directions in elevation and azimuth.'

 lwpeid = NCDF_VARDEF(outfid_lwp, 'clwvi_err',[rfdid], /float)
 NCDF_ATTPUT, outfid_lwp, lwpeid, 'units', 'kg m-2'
 NCDF_ATTPUT, outfid_lwp, lwpeid, 'long_name','standard error of atmosphere mass content of cloud liquid water content at retrieval elevation angle'
 NCDF_ATTPUT, outfid_lwp, lwpeid, '_FillValue',-999.

 c7 = ''
 IF set_lwp_off THEN BEGIN
  c7 = 'clwvi zeroing offset correction based on mwr_pro routine offset_lwp.pro; clear-sky identification clwvi std threshold: ' + STRING(lwp_std_thres, format='(f4.1)') + ' gm-2'
 ENDIF 

 lwpoffzid  = NCDF_VARDEF(outfid_lwp, 'clwvi_offset_zeroing',[timdid], /float)
 NCDF_ATTPUT, outfid_lwp, lwpoffzid, 'units', 'kg m-2'
 NCDF_ATTPUT, outfid_lwp, lwpoffzid, 'long_name','atmosphere_mass_content_of_cloud_liquid_water_content clear sky zeroing offset correction'
 NCDF_ATTPUT, outfid_lwp, lwpoffzid, '_FillValue', -999.
 NCDF_ATTPUT, outfid_lwp, lwpoffzid, 'comment', 'This value has been subtracted from the original clwvi (only zenith!) value to account mainly for instrument calibration drifts. The information is designated for expert user use. ' + c7

 lwpoffid  = NCDF_VARDEF(outfid_lwp, 'clwvi_offset',[timdid], /float)
 NCDF_ATTPUT, outfid_lwp, lwpoffid, 'units', 'kg m-2'
 NCDF_ATTPUT, outfid_lwp, lwpoffid, 'long_name','atmosphere_mass_content_of_cloud_liquid_water_content offset correction based on brightness temperature offset'
 ...
 NCDF_ATTPUT, outfid_lwp, flaid, 'flag_meanings', 'visual_inspection_filter_band_1 visual_inspection_filter_band2 visual_inspection_filter_band3 rain_flag sanity_receiver_band1 sanity_receiver_band2 sun_in_beam tb_threshold_band1 tb_threshold_band2 tb_threshold_band3 lwp_lwp_threshold'
...
;****global attributes
 ...
 NCDF_ATTPUT, outfid_lwp, /GLOBAL, 'Comments', 'clwvi errors are given as a function of retrieval elevation angle. Note, actual observations may occur at different angles. Errors are then interpolated.'
 NCDF_ATTPUT, outfid_lwp, /GLOBAL, 'Licence', ga.l
