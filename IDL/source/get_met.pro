...
; Changes:
; 2015-07-09: 
; Bianca Adler: added variables for reading MET files from HATPRO G4 with Vaisala sensor
;-
verbose=0
...
IF code EQ 599658943 THEN BEGIN
FOR i =0,2 DO BEGIN
...
IF code EQ 599658944 AND n_add EQ 7 THEN BEGIN
 FOR i =0,5 DO BEGIN
   READU,unit,xmin,xmax
   IF verbose THEN print, xmin, xmax
 ENDFOR 
...
time = REPLICATE(0l, n)
rain = REPLICATE(0b, n)
temp = REPLICATE(-999., n)
pres = REPLICATE(-999., n)
humi = REPLICATE(-999., n)
...
IF code EQ 599658944 THEN BEGIN
 x6 = 1E1 ; single precision / 4Byte floating point
 x7 = 1E1 ; single precision / 4Byte floating point
 x8 = 1E1 ; single precision / 4Byte floating point
ENDIF

FOR i =0l,LONG(n-1l) DO BEGIN

 IF code EQ 599658944 AND n_add EQ 7 THEN BEGIN
  READU,unit,x1,x2,x3,x4,x5,x6,x7,x8
 ENDIF ELSE BEGIN
 ...
 pres(i) = x3
 humi(i) = x5
 IF verbose THEN PRINT,i, x1, x2, x3, x4, x5, x6, x7, x8
ENDFOR
