...
c_c,$                 ;cubic coefficients MLR (X frequencies)    (added 2021-07-21)
...
; 2021-07-21 (AW): added cubic regression coefficient 'c_c'
;-
...
ENDIF ELSE IF regression_type EQ 'cubic' THEN BEGIN
 c_l = coefficient_mvr(0:n_freq-1)
 c_q = coefficient_mvr(n_freq:(2*n_freq)-1)
 c_c = coefficient_mvr(2*n_freq:(3*n_freq)-1)
 IF surface_mode EQ 'in_surface' THEN c_s = coefficient_mvr(3*n_freq:(3*n_freq)+2)
ENDIF
