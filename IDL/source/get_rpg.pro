...
tb_spc_struc,$     ;tb variables from spc file (structure): {time:time_tb_scp, tb:tb_spc, el:el_tb_spc, az:az_tb_spc, f:freq_spc, r:rain_tb_spc}
;KEYWORDS
verbose=verbose
...
; *20150710 (UL) 
;  BLB elevation angles now considered to be the same if they are within the +-0.6 angle resolution range. This was a problem for some
;  days since the BLB angles slightly changed from file to file and so more than 6 elevation angles were expected for retrieval application.
; *20180823 (UL)
;  Added stucture for reading spc files
;-
...
;**search and count spc files
;(spc files are RPG specific and are used for spectral consistency checks)

 in_name_spc_today = FILE_SEARCH(raw_path + '*.SPC')
 IF in_name_spc_today(0) EQ '' THEN in_name_spc_today = FILE_SEARCH(raw_path + '*.spc')

 in_name_spc_prev = FILE_SEARCH(raw_path_prev + '*.SPC')
 IF in_name_spc_prev(0) EQ '' THEN in_name_spc_prev = FILE_SEARCH(raw_path_prev + '*.spc')

 in_name_spc = [in_name_spc_prev, in_name_spc_today]
 IF in_name_spc_prev(0) EQ '' THEN in_name_spc = [in_name_spc_today]
 IF in_name_spc_today(0) EQ '' THEN in_name_spc = [in_name_spc_prev]
 n_in_spc = N_ELEMENTS(in_name_spc)
 IF in_name_spc(0) EQ '' THEN n_in_spc = 0

 IF verbose THEN print, 'total number of SPC files found: ', n_in_spc

;**extract data from spc files

 go = 0
 FOR ifile = 0, n_in_spc-1 DO BEGIN
  GET_SPC, in_name_spc(ifile), n_spc, time_spc, rain_spc, freq_spc, tb_spc, az_spc, el_spc, verbose=0

  IF go EQ 0 AND n_spc GT 1 THEN BEGIN

   time_tb_spc = time_spc
   tb_spc2 = tb_spc
   rain_tb_spc = rain_spc
   az_tb_spc = az_spc
   el_tb_spc = el_spc
  ...
  ENDIF ELSE IF go EQ 1 AND n_brt GT 1 THEN BEGIN
   ...
   time_tb_spc = [time_tb_spc, time_spc]
   tb_spc2 = [[tb_spc2], [tb_spc]]
   rain_tb_spc = [rain_tb_spc, rain_spc]
   az_tb_spc = [az_tb_spc, az_spc] 
   el_tb_spc = [el_tb_spc, el_spc]

  ENDIF

 ENDFOR

 IF n_in_spc GT 0 THEN BEGIN 
  tb_spc = tb_spc2

;**convert times
  time_tb_spc = time_tb_spc/86400d
  time_tb_spc = time_tb_spc + JULDAY(1,1,2001,0,0,0)
  i_sort = SORT(time_tb_spc)
  time_tb_spc = time_tb_spc(i_sort)
  rain_tb_spc = rain_tb_spc(i_sort)
  tb_spc = tb_spc(*, i_sort)
  az_tb_spc = az_tb_spc(i_sort)
  el_tb_spc = el_tb_spc(i_sort)
  CALDAT, time_tb_spc, month_tb_spc, day_tb_spc, year_tb_spc, hour_tb_spc, min_tb_spc, sec_tb_spc
  i_day = WHERE(day_tb_spc EQ date_d)

  time_tb_spc = hour_tb_spc(i_day) + min_tb_spc(i_day)/60d + sec_tb_spc(i_day)/3600d
  rain_tb_spc = rain_tb_spc(i_day)
  tb_spc = tb_spc(*, i_day)
  az_tb_spc = az_tb_spc(i_day)
  el_tb_spc = el_tb_spc(i_day)

  tb_spc_struc = {time:time_tb_spc, tb:tb_spc, el:el_tb_spc, az:az_tb_spc, f:freq_spc, r:rain_tb_spc}
...
;*find maximum dimension of el_blb for one day
  ...
  ENDIF ELSE BEGIN
   ...
   FOR k = 0, n_ang_el_blb-1 DO BEGIN
    k_ind = WHERE(el_blb(k) LT ang_el_blb+0.6 AND el_blb(k) GT ang_el_blb-0.6)
...
;(hkd files are RPG specific)
 ...
 FOR ifile = 0, n_in_hkd-1 DO BEGIN
  GET_HKD, in_name_hkd(ifile), tb_struc.f, time_hkd, rec_sanity, verbose=verbose
